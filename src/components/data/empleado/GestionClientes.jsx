...
export default function GestionClientes({
  ...
  const abrirEdicion = (c) => {
    setClienteEdicion(c);
    setForm({ nombre: c.nombre, dni: c.dni || "", telefono: c.telefono, direccion: c.direccion });
    setModalAbierto(true);
  };
  ...
  const guardar = async () => {
    ...
    try {
      if (clienteEdicion) {
        await clienteService.actualizar(clienteEdicion.id, form);
      } else {
        await clienteService.crear(form);
      }
      const data = await clienteService.listar();
      setClientes(data);
      mostrarNotificacion(clienteEdicion ? "Cliente actualizado" : "Cliente registrado");
      setModalAbierto(false);
    } catch (e) {
      mostrarNotificacion(e || "Error al guardar cliente");
    }
  };
